...
Game_MoveAniState::Game_MoveAniState( GameDatas * gd, Entity * selected, std::vector< sf::Vector2i > path ) : m_gameDatas(gd), m_selected(selected), m_path(path), m_speed(3.0f)
{
    init();
}
...
void Game_MoveAniState::draw( sf::RenderTarget & window )
{
    ...
    for(unsigned int i = 0; i < m_gameDatas->entities.size(); ++i) {
        if(m_gameDatas->entities[i] != m_selected) {
